...
class _LiveCamera:
    ...
    def __init__(self, image_lock, debug_lock, run_event, consumer_events, img_shape_out, time_ref, source=0, target_fps=30, target_size=(1080,720), debug_print=False, crop_aspect=None, **kwargs):
        ...
        if good:
            ...
            if crop_aspect is not None:
                if crop_aspect>=1:
                    nh = int(frame.shape[1]/crop_aspect)
                    off = (frame.shape[0]-nh)//2
                    self.sl = np.s_[off:nh+off,:,:]
                else:
                    nw = int(frame.shape[0]*crop_aspect)
                    off = (frame.shape[1]-nw)//2
                    self.sl = np.s_[:,off:nw+off,:]
            else:
                self.sl = np.s_[:]
    ...
    def __del__(self):
        if hasattr(self, 'stream'): self.stream.release()
        if hasattr(self, 'image_sm'): self.image_sm.unlink()
